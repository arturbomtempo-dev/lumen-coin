...
import { PhArrowLeft, PhArrowRight, PhCheckCircle, PhUser, PhIdentificationCard, PhHouse, PhGraduationCap, PhCrown } from '@phosphor-icons/vue'
...
const form = ref({
  ...
  instituicao: instituicoes[0], curso: cursos[0],
})
...
function next() {
  if (step.value < steps.length - 1) {
    ...
    toast.success(`Bem-vindo(a), ${form.value.nome || 'Jogador'}! Sua jornada começa agora.`, { description: 'PERSONAGEM CRIADO! +100 moedas bônus!' })
...
<template>
  <div class="min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-2xl">
      <RouterLink to="/" class="inline-flex items-center gap-2 font-pixel text-[10px] mb-4 hover:text-primary">
      ...
      <PixelCard class="p-6 md:p-8">
        <div class="flex items-center gap-4 mb-6">
          ...
          <div>
            <div class="font-pixel text-[10px] text-primary mb-1">▶ CRIAÇÃO DE PERSONAGEM</div>
        ...
        <div class="mb-6">
          ...
          <div class="h-4 border-2 border-border bg-card overflow-hidden">
            <div
              ...
              :style="{ width: `${progress}%`, backgroundImage: 'repeating-linear-gradient(90deg, hsl(var(--primary)) 0, hsl(var(--primary)) 6px, hsl(var(--primary-shadow)) 6px, hsl(var(--primary-shadow)) 8px)' }"
            />
          </div>
          <div class="flex justify-between mt-3">
            <div v-for="(s, i) in steps" :key="s.label" class="flex flex-col items-center gap-1 flex-1">
              <div
                class="w-8 h-8 flex items-center justify-center border-2 border-border"
                :class="i === step ? 'bg-primary text-primary-foreground animate-glow' : i < step ? 'bg-success text-success-foreground' : 'bg-card'"
              >
                <PhCheckCircle v-if="i < step" weight="fill" :size="16" />
                <component v-else :is="s.icon" weight="fill" :size="16" />
              </div>
              <span class="font-pixel text-[7px]" :class="i === step ? 'text-primary' : 'text-muted-foreground'">{{ s.label }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <template v-if="step === 0">
            ...
            <div>
              ...
              <PixelInput v-model="form.email" type="email" placeholder="voce@email.com" />
            </div>
          </template>
          ...
          <template v-if="step === 2">
            <div class="grid sm:grid-cols-2 gap-4">
              ...
              <div class="sm:col-span-2">
                <label class="font-pixel text-[10px] block mb-2">RUA / LOGRADOURO</label>
              ...
              <div>
                ...
                <PixelInput v-model="form.complemento" placeholder="Apto / Bloco (opcional)" />
              </div>
              <div class="sm:col-span-2">
          ...
          <template v-if="step === 3">
            <div>
              ...
              <select v-model="form.instituicao" class="w-full border-2 border-border bg-card text-card-foreground font-pixel text-[10px] px-3 py-2">
                <option v-for="inst in instituicoes" :key="inst" :value="inst">{{ inst }}</option>
              </select>
            </div>
            <div>
              <label class="font-pixel text-[10px] block mb-2">CURSO</label>
              <select v-model="form.curso" class="w-full border-2 border-border bg-card text-card-foreground font-pixel text-[10px] px-3 py-2">
          ...
          <template v-if="step === 4">
            <label class="font-pixel text-[10px] block mb-3">ESCOLHA SEU PERSONAGEM</label>
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
              ...
              >
                ...
                <div class="font-pixel text-[10px] mt-2">{{ c.nome.toUpperCase() }}</div>
                <div class="font-sans text-[11px] mt-1" :class="personagem === c.id ? 'opacity-90' : 'text-muted-foreground'">{{ c.classe }}</div>
              </button>
            </div>
          </template>
        </div>
        ...
        <div class="flex items-center justify-between mt-8">
          ...
          <PixelButton variant="primary" @click="next">
            {{ step === steps.length - 1 ? 'FINALIZAR' : 'PRÓXIMA FASE' }} <PhArrowRight weight="bold" />
          </PixelButton>
        </div>
      </PixelCard>
    </div>
