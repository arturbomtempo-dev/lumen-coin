...
import { getCompany, updateCompany, deleteCompany } from '@/modules/company/services/company.service';
...
type Tab = 'vantagens' | 'conta';
const tab = ref<Tab>('vantagens');
...
function criar(e: Event) {
    ...
    const nova: VantagemEmpresa = {
        ...
        empresa: authStore.user?.name ?? 'Empresa',
...
const tabs = [
    { id: 'vantagens' as Tab, label: 'VANTAGENS', icon: PhStorefront },
    { id: 'conta' as Tab, label: 'MINHA CONTA', icon: PhUser },
];
                    ...
                    <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
                        ...
                        <div class="flex gap-2 font-pixel text-[9px]">
                            <PixelBadge tone="green">
                                {{ vantagens.filter((v) => v.ativa).length }} ATIVAS
                            </PixelBadge>
                            <PixelBadge tone="gold">
                                {{ vantagens.filter((v) => !v.ativa).length }} INATIVAS
                            </PixelBadge>
                        </div>
                    </div>
