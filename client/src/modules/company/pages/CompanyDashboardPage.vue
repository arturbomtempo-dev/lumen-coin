...
import { PhArrowLeft, PhGameController, PhSun, PhMoon, PhStorefront, PhTicket, PhCheckCircle, PhXCircle, PhPlus, PhEye, PhEyeSlash, PhTrash } from '@phosphor-icons/vue'
...
function criar(e: Event) {
  ...
  const nova: VantagemEmpresa = { id: 'v' + Date.now(), nome: form.value.nome, descricao: form.value.descricao, custo: Number(form.value.custo), empresa: 'Byte Burger', emoji: form.value.emoji, categoria: form.value.categoria, ativa: true }
        ...
        <PixelCard class="p-6">
          ...
          <form class="space-y-4" @submit="criar">
            ...
            <div>
              ...
              <PixelInput v-model="form.descricao" placeholder="Ex: 1 lanche + batata + refri" required />
            </div>
            <div class="grid grid-cols-3 gap-3">
              ...
              <div>
                ...
                <select v-model="form.categoria" class="w-full bg-input border-2 border-border px-3 py-2 font-display text-lg focus:outline-none">
                  <option v-for="c in CATEGORIAS" :key="c">{{ c }}</option>
                </select>
              </div>
            </div>
            <PixelButton variant="success" class="w-full" type="submit"><PhPlus weight="bold" /> PUBLICAR VANTAGEM</PixelButton>
          </form>
        </PixelCard>
        ...
        <PixelCard class="p-6">
          ...
          <div class="space-y-4">
            <div>
              ...
              <PixelInput v-model="codigo" placeholder="LUMEN-XXXXX-XXX" class="uppercase" />
            </div>
            <PixelButton variant="info" class="w-full" @click="validar">VALIDAR CÓDIGO</PixelButton>
            <div
              v-if="resultado"
              class="border-2 border-border p-4 flex items-center gap-3"
              :class="resultado.valido ? 'bg-success text-success-foreground' : 'bg-destructive text-destructive-foreground'"
            >
              <PhCheckCircle v-if="resultado.valido" weight="fill" class="pixel-icon" :size="24" />
              <PhXCircle v-else weight="fill" class="pixel-icon" :size="24" />
              <div>
                <div class="font-pixel text-xs">{{ resultado.valido ? 'CUPOM VÁLIDO' : 'CUPOM INVÁLIDO' }}</div>
                <div class="font-sans text-sm mt-0.5">{{ resultado.msg }}</div>
              </div>
            </div>
            <div class="border-2 border-dashed border-border p-3 font-sans text-xs text-muted-foreground">
              Dica: tente <span class="font-pixel text-[10px]">LUMEN-ABC12-X9K</span> ou qualquer código gerado no app do aluno.
            </div>
          </div>
        </PixelCard>
      </div>
      ...
      <div>
        <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <div class="flex gap-2 font-pixel text-[9px]">
            <PixelBadge tone="green">{{ vantagens.filter(v => v.ativa).length }} ATIVAS</PixelBadge>
            <PixelBadge tone="gold">{{ vantagens.filter(v => !v.ativa).length }} INATIVAS</PixelBadge>
          </div>
        </div>
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <PixelCard v-for="v in vantagens" :key="v.id" class="overflow-hidden" :class="!v.ativa ? 'opacity-60' : ''">
            <div class="aspect-[5/3] flex items-center justify-center bg-hud text-hud-foreground crt-scanlines border-b-2 border-border text-6xl relative">
              <span :class="v.ativa ? 'animate-bob' : ''">{{ v.emoji }}</span>
              <div v-if="!v.ativa" class="absolute top-2 right-2 font-pixel text-[8px] bg-destructive text-destructive-foreground border-2 border-border px-1.5 py-0.5">PAUSADA</div>
            </div>
            <div class="p-4">
              ...
              <p class="font-sans text-xs text-foreground/75 mt-2">{{ v.descricao }}</p>
              <div class="mt-3 flex items-center justify-between">
                <div class="font-pixel text-sm flex items-center gap-1"><CoinIcon :size="12" /> {{ v.custo }}</div>
                <div class="flex gap-2">
                  <PixelButton size="sm" variant="ghost" @click="toggleAtiva(v.id)">
                    <PhEye v-if="!v.ativa" weight="fill" :size="14" />
                    <PhEyeSlash v-else weight="fill" :size="14" />
                  </PixelButton>
                  <PixelButton size="sm" variant="danger" @click="remover(v.id)"><PhTrash weight="fill" :size="14" /></PixelButton>
                </div>
              </div>
            </div>
          </PixelCard>
