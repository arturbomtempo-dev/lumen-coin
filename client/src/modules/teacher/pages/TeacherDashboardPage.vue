...
const enviadas = computed(() =>
    transacoes.value
        .filter((t) => t.professor === 'Prof. Carlos M.')
...
function submit(e: Event) {
    ...
    if (v > disponivel.value)
        return void (err.value = `Você só tem ${disponivel.value} moedas disponíveis este semestre.`);
            ...
            <div class="container flex items-center justify-between py-2 gap-3">
                ...
                <div class="hidden sm:flex items-center gap-4 font-pixel text-[10px]">
                    ...
                    <span class="flex items-center gap-1"
                        ><CoinIcon :size="14" /> {{ disponivel }}/{{ LIMITE }}</span
                    >
                </div>
                <button
            ...
            <div class="grid lg:grid-cols-[1fr_1.3fr] gap-6 items-start">
                <PixelCard class="p-6">
                    ...
                    <form class="space-y-4" @submit="submit">
                        ...
                        <div v-if="err" class="font-pixel text-[10px] text-destructive">
                            ⚠ {{ err }}
                        </div>
            ...
            <section>
                ...
                <div class="mt-4 grid md:grid-cols-[1fr_auto] gap-3 items-end">
                    <div class="flex flex-wrap gap-2">
                        ...
                        >
                            {{ t.id }} ({{ alunos.filter((a) => a.turma === t.id).length }})
                        </button>
                    </div>
                ...
                <div class="mt-5 grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    <PixelCard
                        v-for="t in turmasProfessor.filter(
                            (t) => turmaFiltro === 'todas' || t.id === turmaFiltro
                        )"
                        :key="t.id"
                            ...
                            >
                                ...
                                <div class="flex flex-col items-end gap-1">
                                    <span class="font-pixel text-[10px] flex items-center gap-1"
                                        ><CoinIcon :size="10" /> {{ a.moedas }}</span
                                    >
                                    <button
                                        type="button"
